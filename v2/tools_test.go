...
func TestTools_PushJSONToRemote(t *testing.T) {
	...
	_, _, err := testTool.PushJSONToRemote("https://example.com/some/path", foo, client)
	if err != nil {
...
func TestTools_RandomString(t *testing.T) {
	...
	if len(s) != 10 {
		t.Error("Wrong length returned from Tools.RandomString")
	}
}
...
var slugTests = []struct {
	...
	{name: "complex string", s: "Now?^ is&& the+ time 32", expected: "now-is-the-time-32", errorExpected: false},
	{name: "hindi string and roman characters", s: "Hello worldनमस्ते दुनिया", expected: "hello-world", errorExpected: false},
}
...
var jsonTests = []struct {
	...
	{name: "unknown field in json", json: `{"foooo":"bar"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown field in json", json: `{"foooo":"bar"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
	{name: "missing field name", json: `{foooo:"bar"}`, errorExpected: true, maxSize: 1024, allowUnknown: true},
